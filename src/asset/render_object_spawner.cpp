...
[[nodiscard]] std::optional<Texture> tryGetMaterialBaseColorTexture(const Model& model, std::size_t meshIndex) {
...
[[nodiscard]] glm::mat4 buildModelMatrix(const Model& model, const RenderObjectSpawnRequest& request) {
...
[[nodiscard]] std::optional<SpawnAssets> loadSpawnAssets(const RenderObjectSpawnRequest& request, std::string& error) {
    SpawnAssets assets{};
...
std::vector<rendering::RenderMeshInstance> buildGpuMeshInstances(const SpawnAssets& assets,
    ...
    for (std::size_t meshIndex = 0; meshIndex < assets.model.meshes.size(); ++meshIndex) {
        ...
        for (const Vertex& v : mesh.vertices) {
            interleaved.push_back(rendering::GpuVertex{
                .px = v.position[0], .py = v.position[1], .pz = v.position[2], .nx = v.normal[0], .ny = v.normal[1],
                .nz = v.normal[2], .u = v.texCoord[0],  .v = v.texCoord[1],
            });
        }
        ...
        if (buffers->vao == 0U || buffers->vbo == 0U || buffers->ebo == 0U) {
            LOG_ERROR("OpenGL buffer creation failed for object '{}' mesh {}", request.namePrefix, meshIndex);
            continue;
        }

        glBindVertexArray(buffers->vao);
        glBindBuffer(GL_ARRAY_BUFFER, buffers->vbo);
        glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(interleaved.size() * sizeof(rendering::GpuVertex)),
        ...
        if (request.useModelMaterialTexture) {
            diffuseTexture = tryGetMaterialBaseColorTexture(assets.model, meshIndex).value_or(Texture{});
        }
        ...
        instance.locProjection = assets.program.resource ? assets.program.resource->locProjection : -1;
...
RenderObjectSpawnResult spawnModelAsRenderMeshes(Registry& registry, const RenderObjectSpawnRequest& request) {
    ...
    auto built = buildGpuMeshInstances(*assetsOpt, request, result.textureSelection);
